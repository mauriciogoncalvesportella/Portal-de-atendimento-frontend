...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    SET_ORIGEM (state, payload) { // payload = vetor de origens (raw do banco de dados)
    ...
    CLOSE_TAB_ITEM (state, key) {
      ...
      const index = state.tabControl.items.findIndex(item => item?.id === key)
      if (index >= 0) {
        state.tabControl.tabOpens--
  ...
  actions: {
    ...
    async allSituacao ({ commit, state }, args = { force: false, mutex: false }) {
    ...
    */
    ...
    async allSolicitantesFromChave ({ state, commit }, data = { force: false, cdchave: -1 }) {
    ...
    async userMap ({ state, dispatch, commit }, args = { force: false, mutex: true }) {
      const { force, mutex } = args
    ...
    async getAcaoByCdticket ({ state, commit }, { force = false, cdticket }) {
      if (force || !state.cache.acaoMapByCdticket || !(cdticket in state.cache.acaoMapByCdticket)) {
        const acaoMapTicket = {}
    ...
    async ticketUserMap ({ state, commit }, { cdticketArray, force = false, mutex = true }) {
      if (!state.mutexes.ticketUserMap || !mutex) {
        ...
        if (state.cache.ticketUserMap && !force) {
          cdticketArray = cdticketArray.filter(cdticket => !state.cache.ticketUserMap[cdticket])
        }
    ...
    async situacaoMap ({ state, commit, dispatch }, force = false) {
      ...
      const situacaoMap = {}
      if (force || !state.cache?.ticket?.meta) {
        data = await dispatch('allMeta', { force: true, mutex: false }).situacao
      } else {
    ...
    async urgenciaMap ({ state, commit, dispatch }, force = false) {
      ...
      const urgenciaMap = {}
      if (force || !state.cache?.ticket?.meta) {
        data = await dispatch('allMeta', { force: true, mutex: false }).urgencia
      } else {
